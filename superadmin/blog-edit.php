...
$seoMeta = json_decode($blog['seo_meta'] ?? '{}', true);
// $introdetail = json_decode($blog['introdetail'] ?? '[]', true);
$intro = json_decode($blog['introdetail'] ?? '[]', true);
$sectionsData = json_decode($blog['sections'] ?? '[]', true);
$faqsData = json_decode($blog['faq'] ?? '[]', true);

$highlightWords = json_decode($blog['highlight_words'] ?? '[]', true);
...
<body class="min-h-screen bg-[#f5f4f0] text-[#424649]">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="mb-8">
            <h1 class="display text-3xl font-semibold text-[#424649]">Edit Blog Post</h1>
            <p class="mt-1 text-sm text-[#424649]/55">Edit the details below to publish your article</p>
        </div>
                ...
                <div class="flex-1 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div id="uploadZone" class="relative rounded-xl border-2 border-dashed border-[#d1c9c9] bg-[#fdfcfc] hover:border-[#a22426] hover:bg-[#fdf5f5] transition-all duration-200 cursor-pointer p-10 text-center">
                            ...
                            <div id="upload-placeholder" class="<?= !empty($blog['image']) ? 'hidden' : '' ?>">
                            ...
                            <div id="image-preview" class="<?= !empty($blog['image']) ? 'space-y-3' : 'hidden space-y-3' ?>">
                                <img id="preview-img" src="<?= !empty($blog['image'])
                                                                ? '../uploads/blog/' . htmlspecialchars($blog['image'])
                                                                : '' ?>" class="mx-auto max-h-48 rounded-lg object-cover shadow">
                                <p id="preview-name" class="text-xs text-[#424649]/60">
                                    <?= htmlspecialchars($blog['image'] ?? '') ?>
                                </p>
                                <button
                                    type="button"
                                    onclick="clearImage()"
                                    class="relative z-20 text-xs text-[#a22426] hover:underline cursor-pointer">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="text" name="image_alt" value="<?= htmlspecialchars($seoMeta['image_alt'] ?? '') ?>"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="meta_title" value="<?= htmlspecialchars($seoMeta['meta_title'] ?? '') ?>"
                                ...
                                <div>
                                    ...
                                    <input name="meta_description" value="<?= htmlspecialchars($seoMeta['meta_description'] ?? '') ?>" rows="2"
                            ...
                            <div class="border border-gray-100 rounded-xl p-4 bg-[#fafafa] space-y-4">
                                ...
                                <div class="flex flex-col gap-3">
                                    <div class="flex-1">
                                        ...
                                        <input type="text" name="intro_subheading[]" value="<?= htmlspecialchars($intro[0]['subheading'] ?? '') ?>"
                                            class="w-full rounded-lg px-3.5 py-2.5 text-sm text-[#424649] placeholder-gray-400 border-[1.5px] border-[#e0dcdc] bg-white focus:outline-none focus:border-[#a22426] focus:ring-2 focus:ring-[#a22426]/10 transition-colors">
                                    </div>
                                    <div class="flex-1">
                                        <label class="block text-xs font-medium text-[#424649]/70 uppercase tracking-wide mb-1.5">Secondary Sub Title</label>
                                        <input type="text" name="intro_subtitle[]" value="<?= htmlspecialchars($intro[0]['subtitle'] ?? '') ?>"
                                            class="w-full rounded-lg px-3.5 py-2.5 text-sm text-[#424649] placeholder-gray-400 border-[1.5px] border-[#e0dcdc] bg-white focus:outline-none focus:border-[#a22426] focus:ring-2 focus:ring-[#a22426]/10 transition-colors">
                                    </div>
                                    <div class="w-full">
                                        <label class="block text-xs font-medium text-[#424649]/70 uppercase tracking-wide mb-1.5">Description</label>
                                        <input type="text" name="intro_content[]" value="<?= htmlspecialchars($intro[0]['content'] ?? '') ?>"
                                            class="w-full rounded-lg px-3.5 py-2.5 text-sm text-[#424649] placeholder-gray-400 border-[1.5px] border-[#e0dcdc] bg-white focus:outline-none focus:border-[#a22426] focus:ring-2 focus:ring-[#a22426]/10 transition-colors">
                                    </div>
                                    <div>
                                        ...
                                        <textarea name="highlight_words"
                                            class="w-full rounded-lg px-4 py-3 text-sm border border-[#e0dcdc]"><?= htmlspecialchars(implode("\n", $highlightWords)) ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div id="sections-empty" class="hidden text-center py-8 border-2 border-dashed border-gray-200 rounded-xl">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div id="faq-empty" class="hidden text-center py-8 border-2 border-dashed border-gray-200 rounded-xl">
                            ...
                            <div class="relative">
                                <select name="category_id" required
                                    ...
                                    foreach ($cats as $cat) {
                                        ...
                                        $selected = $cat['id'] == $blog['category_id']
                                            ? 'selected'
                                            : '';
  ...
  px-2 py-1 rounded-lg transition-all duration-150 active:scale-95`;
    ...
    <div id="section-block-${uid}" class="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm">
        ...
        <div class="p-4 sm:p-5 space-y-4">
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    ...
                    <input type="text" name="section_title[]" value="${data.title || ''}"
                           class="${inputCls}">
                </div>
                <div class="sm:w-44">
                    <label class="${labelCls}">TOC Label</label>
                    <input type="text" name="section_toc[]" value="${data.toc || ''}"
                           class="${inputCls}">
                </div>
            </div>
        ...
        // 
        // Existing section data fetch
        // 
        const existingSections = <?= json_encode($sectionsData) ?>;
    ...
    <div id="faq-block-${uid}" class="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm">
        ...
        <div class="p-4 sm:p-5 space-y-4">
            <div>
                ...
                <input type="text" name="faq_question[]"  value="${data.question || ''}"
                       class="${inputCls}">
            </div>
            <div>
        ...
        const existingfaqs = <?= json_encode($faqsData) ?>;
